...
bool rb_sendor_cfg::read_xml( const char* cfg_file ) {
          if ( !xml_.read( cfg_file ) ) {
                    xml_.err( ).clone( err_ );
...
bool rb_routing_sendor_cfg::read_xml( const char* cfg_file ) {
          if ( !xml_.read( cfg_file ) ) {
                    xml_.err( ).clone( err_ );
                    err_.err_no( ERROR_TYPE_RABBITMQ_SENDOR_READ_CFG_FAIL );
                    return false;
...
bool rb_routing_sendor_cfg::check_rb_type( int type ) {
          return (( ( rb_cfg_type ) type ) == RB_CFG_TYPE_ROUTING ) ? true : false;
}
...
rb_topic_sendor_cfg::rb_topic_sendor_cfg( ) : rb_routing_sendor_cfg( ) {

}

rb_topic_sendor_cfg::~rb_topic_sendor_cfg( ) {

}

bool rb_topic_sendor_cfg::check_rb_type( int type ) {
          return (( ( rb_cfg_type ) type ) == RB_CFG_TYPE_TOPIC ) ? true : false;
}