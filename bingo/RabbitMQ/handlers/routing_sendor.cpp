...
#include "routing_sendor.h"

// --------------------------- rabbitmq logger  ------------------------- //

log_handler* rb_routing_sendor_logger;

// --------------------------- rabbitmq sendor thread ------------------------- //

void make_routing_sendor_thr( bingo::RabbitMQ::config::rb_routing_sendor_cfg_value*& p ) {
          ...
          while ( true ) {
                    ...
                    AsioHandler handler( ioService , rb_routing_sendor_logger );
                    ...
                    channel.declareExchange( p->exchange.c_str( ) , AMQP::direct ).onSuccess( [&]( ) {
                              //                                                channel.publish(p->exchange.c_str(), "", msg);
                              if ( rb_routing_sendor_logger ) {
                                        string s_msg = "exchange declare success!";
                                        rb_routing_sendor_logger->handle( bingo::log::LOG_LEVEL_INFO , RABBITMQ_ROUTING_SENDOR_INFO , s_msg );
                              }
                    } );
...
class rb_routing_sendor_task : public bingo::thread::many_to_one<rb_data_message , rb_exit_message> {
public:

          rb_routing_sendor_task( thr_top_callback& f ) :
          bingo::thread::many_to_one<rb_data_message , rb_exit_message>( f ) {

          }

          virtual ~rb_routing_sendor_task( ) {
          }
};
typedef bingo::singleton_v1<rb_routing_sendor_task ,
rb_routing_sendor_task::thr_top_callback
> RB_ROUTING_SENDOR_TASK_TYPE;

// --------------------------- rabbitmq task callback ------------------------- //

using namespace bingo::RabbitMQ::handlers;

void routing_sendor::rb_top( rb_data_message*& msg , bingo_empty_type& ) {
          ...
          if ( is_valid ) {
                    ...
                    if ( p == 0 ) {
                              ...
                              if ( rb_routing_sendor_logger ) {
                                        ...
                                        s_msg.append( s_key );
                                        rb_routing_sendor_logger->handle( bingo::log::LOG_LEVEL_ERROR , RABBITMQ_ROUTING_SENDOR_ERROR , s_msg );
                              }
                    } else {
                              // Check AMQP::Channel*.
                              if ( p->channel == 0 ) {
                              ...
                              if ( !re ) {
                                        ...
                                        if ( rb_routing_sendor_logger ) {
                                                  ...
                                                  rb_routing_sendor_logger->handle( bingo::log::LOG_LEVEL_DEBUG , RABBITMQ_ROUTING_SENDOR_INFO , s_msg );
                                        }
#endif
                              }
...
void routing_sendor::transfer_data_by_routingkey( string& routingkey , char*& in , size_t& in_size ) {

}

void routing_sendor::transfer_data_by_key( string& key , char*& in , size_t& in_size ) {
          ...
          if ( RB_ROUTING_SENDOR_TASK_TYPE::instance( )->put( data , err ) == -1 ) {
                    if ( rb_routing_sendor_logger ) {
...
routing_sendor::routing_sendor( const char* cfg_file ) : rb_sendor( ) {
          ...
          if ( !is_valid ) {
                    ...
                    RB_ROUTING_SENDOR_TASK_TYPE::construct(
                            bind( &routing_sendor::rb_top , this , _1 , _2 ) // thread_task queue top callback
                            );
          }
}

routing_sendor::~routing_sendor( ) {
          // TODO Auto-generated destructor stub
          if ( is_valid ) RB_ROUTING_SENDOR_TASK_TYPE::release( );
}

void routing_sendor::connet_to_server( log_handler*& log ) {
          if ( is_valid ) {

                    rb_routing_sendor_logger = log;
                    ...
                    for ( int i = 0; i < max; i++ ) {
                              ...
                              if ( p->type == config::RB_CFG_TYPE_ROUTING )
                                        tg_.create_thread( bind( make_routing_sendor_thr , p ) );
